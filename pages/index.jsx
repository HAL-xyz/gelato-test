...
const notify = async (web3) => Notify({
...
const Home = () => {
  ...
  const getGelatoBalance = () => {
    ...
    (async () => {
      const ethGelatoBalance = await getBalanceProxyForUser(walletAddress, web3);
  ...
  const deposit = () => {
    ...
    (async () => {
      try {
        setPendingTx(true);
        const { hash, promiEvent } = await depositToProxy(depositAmount, walletAddress, web3);
        (await notify(web3)).hash(hash);
  ...
  const withdraw = () => {
    ...
    (async () => {
      try {
        setPendingTx(true);
        const { hash, promiEvent } = await withdrawFromProxy(withdrawAmount, walletAddress, web3);
        (await notify(web3)).hash(hash);
  ...
  const createTaks = () => {
    ...
    if (chiTokenAmount >= 1 && chiTokenAmount <= 140) {
      (async () => {
        try {
          setPendingTx(true);
          const { hash, promiEvent } = await submitTask(chiTokenAmount, gasThreshold, walletAddress, web3);
          (await notify(web3)).hash(hash);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { cpk } = await getProxyForUser(walletAddress, web3);
      setGelatoAccount(cpk.address);
    })();
  }, [walletAddress]);
  ...
  return (
    <>
      <div className="container mx-auto px-8">
        <h1 className="font-serif text-2xl mt-8">
          🚧 Gelato Integration Demo
        </h1>
        <h2 className="text-lg">
          Please, use a Rinkeby account 😉
        </h2>
        {currentGasValue
          && (
            <h2 className="text-lg">
            ...
            <>
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    className={`
                      ${(!gelatoAccount || pendingTx) ? 'bg-gray-300' : 'bg-purple-600'}
